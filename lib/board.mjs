...
// --- STOCK PRICE LOOKUP (for /price route) ---

/**
 * Look up a stock's realtime data from the board cache.
 * Returns the raw SSI stock object or null if not found.
 */
export function lookupStockFromCache(symbol) {
  for (const [, stocks] of boardTabCache) {
    if (!Array.isArray(stocks)) continue;
    const found = stocks.find(
      (s) => (s.stockSymbol ?? s.code ?? "").toUpperCase() === symbol
    );
...
/**
 * Fetch a stock price from SSI by searching all exchanges.
 * Returns the raw SSI stock object or null.
 */
export async function fetchStockFromSSI(symbol) {
  const tabs = ["hose", "hnx", "upcom"];
  for (const tab of tabs) {
    let data = boardTabCache.get(tab);
    if (!data) data = await fetchWithTimeout(BOARD_TAB_URLS[tab]);
